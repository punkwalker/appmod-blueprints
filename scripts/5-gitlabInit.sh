...
# Uses existing GitLab token from AWS Secrets Manager (created by 1-argocd-gitlab-setup.sh)

export TIMEOUT=10
export RETRY_INTERVAL=10
export MAX_RETRIES=40

# Get GitLab configuration from Secrets Manager (created by 1-argocd-gitlab-setup.sh)
echo "Retrieving GitLab configuration from AWS Secrets Manager..."
GITLAB_SECRET=$(aws secretsmanager get-secret-value --secret-id "${RESOURCE_PREFIX:-peeks}-gitlab-pat" --region $AWS_REGION --query 'SecretString' --output text)

if [ -z "$GITLAB_SECRET" ]; then
    echo "Error: GitLab token not found in Secrets Manager. Run 1-argocd-gitlab-setup.sh first."
    exit 1
fi

export GITLAB_TOKEN=$(echo "$GITLAB_SECRET" | jq -r '.token')
export GITLAB_HOSTNAME=$(echo "$GITLAB_SECRET" | jq -r '.hostname')
export USERNAME=$(echo "$GITLAB_SECRET" | jq -r '.username')
