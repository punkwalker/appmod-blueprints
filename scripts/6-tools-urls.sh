...
# Export environment variables and store in ~/.bashrc.d/env.sh
print_header "Exporting Environment Variables"
print_info "Creating environment variables in ~/.bashrc.d/env.sh..."

# Create the ~/.bashrc.d directory if it doesn't exist
mkdir -p ~/.bashrc.d

# Create or update the env.sh file with the variables
cat >> ~/.bashrc.d/env.bash << EOF
# Tool URLs and Credentials - Generated by 6-tools-urls.sh
export KEYCLOAKIDPPASSWORD="$USER_PASSWORD"
export BACKSTAGEURL="$BACKSTAGE_URL"
export GITLABPW="$IDE_PASSWORD"
export GITLABURL="$GITLAB_URL"
export ARGOCDPW="$IDE_PASSWORD"
export ARGOCDURL="$ARGOCD_URL"
export GRAFANA_WORKSPACE_URL=""  # To be populated when Grafana is available
export ARGOWFURL="$WORKFLOWS_URL"
EOF

# Source the file to make variables available in current session
source ~/.bashrc.d/env.sh

print_info "Environment variables exported:"
print_info "KEYCLOAKIDPPASSWORD: ${BOLD}$KEYCLOAKIDPPASSWORD${NC}"
print_info "BACKSTAGEURL: ${BOLD}$BACKSTAGEURL${NC}"
print_info "GITLABPW: ${BOLD}$GITLABPW${NC}"
print_info "GITLABURL: ${BOLD}$GITLABURL${NC}"
print_info "ARGOCDPW: ${BOLD}$ARGOCDPW${NC}"
print_info "ARGOCDURL: ${BOLD}$ARGOCDURL${NC}"
print_info "ARGOWFURL: ${BOLD}$ARGOWFURL${NC}"
print_info "Variables saved to: ${BOLD}~/.bashrc.d/env.sh${NC}"