resource "aws_cloudwatch_dashboard" "cni-helper-cw-dashboard" {
  dashboard_name = "Peeks_Management_Vpc_Cni"
...
resource "aws_cloudwatch_dashboard" "karpenter-cw-dashboard" {
  dashboard_name = "Peeks_Management_Karpenter"
  dashboard_body = replace(
...
resource "aws_cloudwatch_dashboard" "coredns-cw-dashboard" {
  dashboard_name = "Peeks_Management_CoreDNS"
  dashboard_body = replace(
...
################################################################################
resource "aws_sns_topic" "peeks-alerts" {
  name = "peeks-alerts"
}

resource "aws_sqs_queue" "peeks_alerts_queue" {
  name = "peeks_alerts"
}

resource "aws_sns_topic_subscription" "peeks_alerts_sqs_target" {
  topic_arn = aws_sns_topic.peeks-alerts.arn
  protocol  = "sqs"
  endpoint  = aws_sqs_queue.peeks_alerts_queue.arn
}
